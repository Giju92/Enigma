...
public class Book : MonoBehaviour {
    ...
    public Sprite[] unlockedPages;
    bool enableShadowEffect = false;
    ...
    public void DragRightPageToPoint(Vector3 point) {
        ...
        if (GameManager.Instance.IsSolved((currentPage) / 2))
            Left.sprite = unlockedPages[currentPage];
        else
            Left.sprite = bookPages[currentPage];
        ...
        if (currentPage < bookPages.Length - 2) {
            if (GameManager.Instance.IsVisited((currentPage + 2) / 2))
                Right.sprite = unlockedPages[currentPage + 1];
            else
                Right.sprite = bookPages[currentPage + 1];
        }
        if (currentPage < bookPages.Length - 2) {
            if (GameManager.Instance.IsSolved((currentPage + 2) / 2))
                RightNext.sprite = unlockedPages[currentPage + 2];
            else
                RightNext.sprite = bookPages[currentPage + 2];
        }
    ...
    public void DragLeftPageToPoint(Vector3 point) {
        ...
        if (GameManager.Instance.IsVisited((currentPage) / 2))
            Right.sprite = unlockedPages[currentPage - 1];
        else
            Right.sprite = bookPages[currentPage - 1];
        ...
        if (currentPage >= 2) {
            if (GameManager.Instance.IsSolved((currentPage - 2) / 2))
                Left.sprite = unlockedPages[currentPage - 2];
            else
                Left.sprite = bookPages[currentPage - 2];

            if (GameManager.Instance.IsVisited((currentPage - 2) / 2))
                LeftNext.sprite = unlockedPages[currentPage - 3];
            else
                LeftNext.sprite = bookPages[currentPage - 3];
    ...
    void UpdateSprites() {
        if (GameManager.Instance.IsVisited(currentPage / 2))
            LeftNext.sprite = unlockedPages[currentPage - 1];
        else
            LeftNext.sprite = bookPages[currentPage - 1];

        if (GameManager.Instance.IsSolved(currentPage / 2))
            RightNext.sprite = unlockedPages[currentPage];
        else
